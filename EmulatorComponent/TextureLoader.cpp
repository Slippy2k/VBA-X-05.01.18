...
using namespace EmulatorComponent;

void LoadTextureFromFile(ID3D11Device1 *device, const wchar_t *filename, ID3D11Resource **texture, ID3D11ShaderResourceView **textureSRV, Rectangle &bounds)
{
	/*FILE *file;
	...
	int totalRead = 0;
	int read;
	while((read = fread(bytes + totalRead, 1, fileSize - totalRead, file)) > 0)
	{
		totalRead += read;
	}
	...
	/*if (fgets(bytes, fileSize, file) == nullptr)
	{
		fclose(file);
		delete[] bytes;
		return;
	...
	bounds.X = 0;
	bounds.Y = 0;
	int width = bounds.Width = header->dwWidth;
	int height = bounds.Height = header->dwHeight;
